...
async function updateProjectInGoogleSheet(project: any): Promise<boolean> {
  ...
  const spreadsheetId = process.env.GOOGLE_PROJECTS_SPREADSHEET_ID || process.env.GOOGLE_SPREADSHEET_ID || DEFAULT_SPREADSHEET_ID;
...
async function syncSubmissionsFromGoogleSheet(): Promise<any[] | null> {
  const spreadsheetId = process.env.GOOGLE_LOGS_SPREADSHEET_ID || process.env.GOOGLE_SPREADSHEET_ID || DEFAULT_SPREADSHEET_ID;
  if (!spreadsheetId) return null;
...
app.get("/api/config-status", async (req, res) => {
  ...
  res.json({
    ...
    serviceAccountEmail,
    projectsSpreadsheetId: process.env.GOOGLE_PROJECTS_SPREADSHEET_ID || process.env.GOOGLE_SPREADSHEET_ID || DEFAULT_SPREADSHEET_ID,
    logsSpreadsheetId: process.env.GOOGLE_LOGS_SPREADSHEET_ID || process.env.GOOGLE_SPREADSHEET_ID || DEFAULT_SPREADSHEET_ID,
